...
if mode == "a":
    ...
    existing_files = [f for f in os.listdir(output_dir) if f.startswith("config_") and f.endswith(".yaml")]
    max_idx = max([int(f.split("_")[1].split(".")[0]) for f in existing_files], default=-1)
    start_idx = max_idx + 1
else:
...
for i in range(start_idx, start_idx + num_configs):
    ...
    config = {
        ...
        "element_sigmas": {7: float(rng.uniform(0.1, 0.5))},
...
print(f"Configs successfully {'appended' if mode == 'a' else 'written'} to {output_dir}")
