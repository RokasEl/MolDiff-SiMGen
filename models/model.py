...
class MolDiff(Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.node_embedder = nn.Linear(
            num_node_types, node_dim - time_dim, bias=False
        )  # element type
        self.edge_embedder = nn.Linear(
            num_edge_types, edge_dim - time_dim, bias=False
        )  # bond type
    ...
    def define_betas_alphas(self, config):
        ...
        if self.categorical_space == "continuous":
            self.scaling = getattr(config, "scaling", [1.0, 1.0, 1.0])
        else:
            self.scaling = [
                1.0,
                1.0,
                1.0,
            ]  # actually not used for discrete space (defined for compatibility)

        # # diffusion for pos
        pos_betas = get_beta_schedule(
            num_timesteps=self.num_timesteps, **config.diff_pos
        )
        ...
        edge_betas = get_beta_schedule(
            num_timesteps=self.num_timesteps, **config.diff_bond
        )
        if self.categorical_space == "discrete":
            init_prob = config.diff_bond.init_prob
            self.edge_transition = GeneralCategoricalTransition(
                edge_betas, self.num_edge_types, init_prob=init_prob
            )
        elif self.categorical_space == "continuous":
            scaling_edge = self.scaling[2]
            self.edge_transition = ContigousTransition(
                edge_betas, self.num_edge_types, scaling_edge
            )
        else:
            raise ValueError(self.categorical_space)
    ...
    def add_noise(
        ...
        **kwargs,
        ...
        halfedge_pert = self.edge_transition.add_noise(
            halfedge_type, time_step, batch_halfedge
        )
    ...
    def get_loss(
        ...
        edge_index = torch.cat(
            [halfedge_index, halfedge_index.flip(0)], dim=1
        )  # undirected edges
        batch_edge = torch.cat([batch_halfedge, batch_halfedge], dim=0)
        if self.categorical_space == "discrete":
            h_node_pert, log_node_t, log_node_0 = node_pert
        ...
        if self.bond_len_loss == True:
            ...
            true_length = torch.norm(
                node_pos[bond_index[0]] - node_pos[bond_index[1]], dim=-1
            )
            pred_length = torch.norm(
                pred_pos[bond_index[0]] - pred_pos[bond_index[1]], dim=-1
            )
        ...
        if self.categorical_space == "discrete":
            ...
            log_node_post_true = self.node_transition.q_v_posterior(
                log_node_0, log_node_t, time_step, batch_node, v0_prob=True
            )
            log_node_post_pred = self.node_transition.q_v_posterior(
                log_node_recon, log_node_t, time_step, batch_node, v0_prob=True
            )
            kl_node = self.node_transition.compute_v_Lt(
                log_node_post_true,
                log_node_post_pred,
                log_node_0,
                t=time_step,
            ...
            log_edge_post_true = self.edge_transition.q_v_posterior(
                log_halfedge_0, log_halfedge_t, time_step, batch_halfedge, v0_prob=True
            )
            log_edge_post_pred = self.edge_transition.q_v_posterior(
                log_halfedge_recon,
                log_halfedge_t,
                time_step,
                batch_halfedge,
            ...
            kl_edge = self.edge_transition.compute_v_Lt(
                log_edge_post_true,
                log_edge_post_pred,
                log_halfedge_0,
                t=time_step,
        ...
        # total
        loss_total = (
            loss_pos + loss_node + loss_edge + (loss_len if self.bond_len_loss else 0)
        )
    ...
    def forward(
        self, h_node_pert, pos_pert, batch_node, h_edge_pert, edge_index, batch_edge, t
    ):
        """
        ...
        h_node_pert = torch.cat(
            [self.node_embedder(h_node_pert), time_embed_node], dim=-1
        )
        time_embed_edge = self.time_emb(t.index_select(0, batch_edge))
        h_edge_pert = torch.cat(
            [self.edge_embedder(h_edge_pert), time_embed_edge], dim=-1
        )
