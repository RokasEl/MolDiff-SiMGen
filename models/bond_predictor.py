...
class BondPredictor(Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.edge_embedder = nn.Linear(
            num_node_types * 2, edge_dim - time_dim, bias=False
        )  # the init edge features
        ...
        self.edge_decoder = MLP(
            edge_dim + node_dim, num_edge_types, edge_dim, num_layer=3
        )

        self.edge_weight = torch.tensor(
            [0.1] + [1.0] * (self.num_edge_types - 1), dtype=torch.float32
        )
    ...
    def define_betas_alphas(self, config):
        ...
        if self.categorical_space == "continuous":
            ...
            self.scaling = [
                ...
                1.0,
                1.0,
            ]  # actually not used for discrete space (define for compatibility)
    ...
    def get_loss(
        ...
        if self.num_timesteps != 0:
            ...
            h_node = node_pert[
                0
            ]  # compatible for both discrete and continuous catergorical_space
        else:
        ...
        pred_halfedge = self(
            h_node, pos_node, batch_node, edge_index, batch_edge, time_step
        )
    ...
    def forward(self, h_node, pos_node, batch_node, edge_index, batch_edge, t):
        """
        ...
        if self.num_timesteps != 0:
            ...
            t = torch.zeros(
                batch_node.max() + 1, device=pos_node.device, dtype=torch.long
            )
        ...
        h_node, _, h_edge = self.encoder(
            ...
            node_time=t.index_select(0, batch_node).unsqueeze(-1)
            / max(self.num_timesteps, 1),
            edge_time=t.index_select(0, batch_edge).unsqueeze(-1)
            / max(self.num_timesteps, 1),
        )
        ...
        h_edge_extend = torch.cat(
            [
                ...
                h_node[edge_index[0, :n_halfedges]]
                + h_node[edge_index[1, :n_halfedges]],
            ],
            dim=-1,
        )
