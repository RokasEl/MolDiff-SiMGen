...
class NodeBlock(Module):
    def __init__(self, node_dim, edge_dim, hidden_dim, use_gate):
        ...
        if self.use_gate:
            self.gate = MLP(
                edge_dim + node_dim + 1, hidden_dim, hidden_dim
            )  # add 1 for time
    ...
    def __init__(
        ...
        if dist_version == "new":
            ...
            self.distance_expansion = GaussianSmearing(
                stop=cutoff, num_gaussians=edge_dim - self.additional_edge_feat
            )
...
class BondFFN(Module):
    ...
    def forward(self, bond_feat_input, node_feat_input, time):
        ...
        if self.use_gate:
            gate = self.gate(
                torch.cat([bond_feat_input, node_feat_input, time], dim=-1)
            )
...
class BondBlock(Module):
    def __init__(
        ...
        self.bond_ffn_left = BondFFN(
            bond_dim, node_dim, inter_dim=inter_dim, use_gate=use_gate
        )
    ...
    def forward(self, h_bond, bond_index, h_node, atten_index=None):
        ...
        if self.use_atten:
            (
                index_query_bond_left,
                index_key_bond_left,
                index_query_bond_right,
                index_key_bond_right,
            ) = atten_index
            ...
            h_bond = scatter_sum(
                (alpha.unsqueeze(-1) * values_j).view(num_attns, -1),
                index_query_bond_left,
                dim=0,
                dim_size=h_bond.size(0),
            )
...
class EdgeBlock(Module):
    def __init__(self, edge_dim, node_dim, hidden_dim=None, use_gate=True):
        ...
        self.bond_ffn_left = BondFFN(
            edge_dim, node_dim, inter_dim=inter_dim, use_gate=use_gate
        )
        self.bond_ffn_right = BondFFN(
            edge_dim, node_dim, inter_dim=inter_dim, use_gate=use_gate
        )
...
class NodeEdgeNet(Module):
    def __init__(self, node_dim, edge_dim, num_blocks, cutoff, use_gate, **kwargs):
        ...
        for _ in range(num_blocks):
            self.node_blocks_with_edge.append(
                NodeBlock(
                    node_dim=node_dim,
                    edge_dim=edge_dim,
                    hidden_dim=node_dim,
    ...
    def forward(self, h_node, pos_node, h_edge, edge_index, node_time, edge_time):
        for i in range(self.num_blocks):
            ...
            if self.update_pos or (i == 0):
                h_edge_dist, relative_vec, distance = self._build_edges_dist(
                    pos_node, edge_index
                )
            if self.update_edge:
            ...
            h_node_with_edge = self.node_blocks_with_edge[i](
                h_node, edge_index, h_edge, node_time
            )
            if self.update_edge:
                h_edge = h_edge + self.edge_blocks[i](
                    h_edge, edge_index, h_node, edge_time
                )
            h_node = h_node + h_node_with_edge
            # pos updates
            if self.update_pos:
                pos_node = pos_node + self.pos_blocks[i](
                    h_node, h_edge, edge_index, relative_vec, distance, edge_time
                )
    ...
    def forward(self, h_node, h_edge, edge_index, relative_vec, distance, edge_time):
        ...
        force_edge = (
            ...
            * relative_vec
            ...
            / (distance.unsqueeze(-1) + 1.0)
        )
        delta_pos = scatter_sum(
            force_edge, edge_index_left, dim=0, dim_size=h_node.shape[0]
        )
...
class NodeBondNet(Module):
        ...
        for _ in range(num_blocks):
            self.node_blocks_with_edge.append(
                NodeBlock(
                    ...
                    num_heads=None,
                    use_atten=False,
                    use_gate=use_gate,  # never use atten for edges message becused too many edges
                )
            )
            self.node_blocks_with_bond.append(
                NodeBlock(
                    node_dim=node_dim,
                    edge_dim=bond_dim,
                    key_dim=None,
    ...
    def forward(
        ...
        for i in range(self.num_blocks):
            ...
            h_node_with_edge = self.node_blocks_with_edge[i](
                h_node, edge_index, edge_attr
            )
            if self.bond_dim > 0:
                # node updates with bonds
                h_node_with_bond = self.node_blocks_with_bond[i](
                    h_node, bond_index, h_bond
                )
                # bond updates
    ...
    def _build_bond_atten(self, bond_index):
        ...
        for node in torch.unique(left_node):
            ...
            idx_query_bond, idx_key_bond = torch.meshgrid(
                idx_connect_left, idx_connect_left, indexing="ij"
            )
            ...
            idx_query_bond, idx_key_bond = torch.meshgrid(
                idx_connect_right, idx_connect_right, indexing="ij"
            )
    ...
    def _build_bond_atten2(self, bond_index):
        ...
        for node in left_node_unique:
            # ind_connect_left = (left_node == node)
            # idx_connect_left = torch.nonzero(ind_connect_left)[:, 0]
            idx_connect_left = torch.LongTensor(group2node_dict_left[node]).to(
                bond_index.device
            )
            ...
            # ind_connect_right = (right_node == node)
            # idx_connect_right = torch.nonzero(ind_connect_right)[:, 0]
            idx_connect_right = torch.LongTensor(group2node_dict_right[node]).to(
                bond_index.device
            )
...
class PosPredictor(Module):
    ...
    def forward(
        self, h_node, pos_node, h_bond, bond_index, h_edge, edge_index, is_frag
    ):
        ...
        force_edge = weight_edge * (
            pos_node[edge_index_left] - pos_node[edge_index_right]
        )
        delta_pos = scatter_sum(
            force_edge, edge_index_left, dim=0, dim_size=h_node.shape[0]
        )
        ...
        if self.bond_dim > 0:
            ...
            force_bond = weight_bond * (
                pos_node[bond_index_left] - pos_node[bond_index_right]
            )
            delta_pos = delta_pos + scatter_sum(
                force_bond, bond_index_left, dim=0, dim_size=h_node.shape[0]
            )

        pos_update = pos_node + delta_pos / 10.0
