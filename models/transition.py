...
class ContigousTransition(nn.Module):
    def __init__(self, betas, num_classes=None, scaling=1.0):
        super().__init__()
        ...
        self.coef_x0 = to_torch_const(
            np.sqrt(alphas_bar_prev) * betas / (1 - alphas_bar)
        )
        self.coef_xt = to_torch_const(
            np.sqrt(alphas) * (1 - alphas_bar_prev) / (1 - alphas_bar)
        )
        self.std = to_torch_const(
            np.sqrt((1 - alphas_bar_prev) * betas / (1 - alphas_bar))
        )
...
class CategoricalTransition(nn.Module):
    ...
    def q_vt_pred(self, log_v0, t, batch):
        ...
        log_probs = log_add_exp(
            log_v0 + log_alpha_bar_t, log_1_min_alpha_bar - np.log(self.num_classes)
        )
        return log_probs
...
class GeneralCategoricalTransition(nn.Module):
    ...
    def q_vt_pred(self, log_v0, t, batch):
        ...
        return torch.log(q_vt + self.eps).clamp_min(-32.0)
    ...
    def q_v_posterior(self, log_v0, log_vt, t, batch, v0_prob):
        ...
        out = torch.log(fact1 + self.eps).clamp_min(-32.0) + torch.log(
            fact2 + self.eps
        ).clamp_min(-32.0)
    ...
    def sample_init(self, n):
        ...
        init_log_atom_vt = (
            torch.log(torch.from_numpy(self.init_prob) + self.eps)
            .clamp_min(-32.0)
            .to(self.q_mats.device)
        )
