...
def print_pool_status(pool, logger):
    logger.info(
        "[Pool] Finished %d | Failed %d" % (len(pool.finished), len(pool.failed))
    )
...
def data_exists(data, prevs):
    for other in prevs:
        if len(data.logp_history) == len(other.logp_history):
            if (
                (data.ligand_context_element == other.ligand_context_element)
                .all()
                .item()
                and (
                    data.ligand_context_feature_full
                    == other.ligand_context_feature_full
                )
                .all()
                .item()
                and torch.allclose(data.ligand_context_pos, other.ligand_context_pos)
            ):
                return True
    return False
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default="./configs/sample/sample_MolDiff.yml"
    )
    ...
    # # Logging
    log_root = (
        args.outdir.replace("outputs", "outputs_vscode")
        if sys.argv[0].startswith("/data")
        else args.outdir
    )
    ...
    if "guidance" in config.sample:
        guidance = (
            config.sample.guidance
        )  # tuple: (guidance_type[entropy/uncertainty], guidance_scale)
    else:
        guidance = None
    ...
    while len(pool.finished) < config.sample.num_mols:
        ...
        # prepare batch
        batch_size = (
            args.batch_size if args.batch_size > 0 else config.sample.batch_size
        )
        n_graphs = min(batch_size, (config.sample.num_mols - len(pool.finished)) * 2)
        batch_holder = make_data_placeholder(
            n_graphs=n_graphs, device=args.device, max_size=max_size
        )
                        ...
                        try:
                            mol_traj.append(
                                reconstruct_from_generated_with_edges(
                                    traj_info[t], False, add_edge=add_edge
                                )
                            )
                        except MolReconsError:
        ...
        with open(os.path.join(log_dir, "SMILES.txt"), "a") as smiles_f:
            for i, data_finished in enumerate(gen_list):
                ...
                Chem.MolToMolFile(
                    rdmol, os.path.join(sdf_dir, "%d.sdf" % (i + len(pool.finished)))
                )

                if "traj" in data_finished:
                    with Chem.SDWriter(
                        os.path.join(sdf_dir, "traj_%d.sdf" % (i + len(pool.finished)))
                    ) as w:
