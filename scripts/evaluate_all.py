...
def load_mols_from_generated(exp_name, result_root):
    ...
    assert sdf_dir.exists(), (
        f"No sdf directory of sampling with prefix `{exp_name}` and suffix `_SDF` in {result_root}"
    )
    ...
    mol_names = [
        mol_name
        for mol_name in os.listdir(sdf_dir)
        if (mol_name[-4:] == ".sdf") and ("traj" not in mol_name)
    ]
    mol_ids = np.sort([int(mol_name[:-4]) for mol_name in mol_names])

    # load sdfs
    mol_dict_raw = {
        mol_id: Chem.MolFromMolFile(os.path.join(sdf_dir, "%d.sdf" % mol_id))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exp_name",
        type=str,
        default="sample_MolDiff_20230602",
    ...
    if from_where == "dataset":
        ...
        mol_dict, df, metrics_dir, df_path = load_mols_from_generated(
            exp_name, args.result_root
        )
    ...
    for metric_name in metrics_list:
        ...
        if metric_name in [
            ...
            if list(results_list[0].keys())[0] not in df.columns:
                df = pd.concat(
                    [df, pd.DataFrame(results_list, index=mol_dict.keys())], axis=1
                )
            else:
            ...
            lengths = local3d.calc_frequent(
                mol_dict.values(), type_="length", parallel=False
            )
            logger.info(f"Computing local 3d - bond angles metric...")
            angles = local3d.calc_frequent(
                mol_dict.values(), type_="angle", parallel=False
            )
            logger.info(f"Computing local 3d - dihedral angles metric...")
            dihedral = local3d.calc_frequent(
                mol_dict.values(), type_="dihedral", parallel=False
            )
            ...
            fingerprints = [Chem.RDKFingerprint(mol) for mol in mol_dict.values()]
            diversity = sim.get_diversity(fingerprints)
            uniqueness["diversity"] = diversity
            sim_with_val = sim.get_sim_with_val(fingerprints)
            uniqueness["sim_with_val"] = sim_with_val
            sim_with_train = sim.get_sim_with_train(fingerprints)
            uniqueness["sim_with_train"] = sim_with_train
