...
class FeaturizeMol(object):
    def __init__(self, atomic_numbers, mol_bond_types, use_mask_node, use_mask_edge):
        super().__init__()
        ...
        self.num_edge_types = (
            self.num_bond_types + 1 + int(use_mask_edge)
        )  # + 1 for the non-bonded edges
    ...
    def __call__(self, data: Drug3DData):
        ...
        # node type
        assert np.all([ele in self.atomic_numbers for ele in data.element]), (
            "unknown element"
        )
        data.node_type = torch.LongTensor(
            [self.ele_to_nodetype[ele.item()] for ele in data.element]
        )
        ...
        for i in range(
            data.num_bonds * 2
        ):  # multiplication by two is for symmtric of bond index
            edge_type_mat[data.bond_index[0, i], data.bond_index[1, i]] = (
                data.bond_type[i]
            )
...
def make_data_placeholder(n_graphs, device=None, max_size=None):
    ...
    if max_size is None:  # use statistics from GEOM-Drug dataset
        n_nodes_list = np.random.normal(
            24.923464980477522, 5.516291901819105, size=n_graphs
        )
    else:
        n_nodes_list = np.array([max_size] * n_graphs)
    n_nodes_list = n_nodes_list.astype("int64")
    batch_node = np.concatenate(
        [np.full(n_nodes, i) for i, n_nodes in enumerate(n_nodes_list)]
    )
    halfedge_index = []
    batch_halfedge = []
    idx_start = 0
