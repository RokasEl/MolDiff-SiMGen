...
periodic_table = {6: "C", 7: "N", 8: "O", 9: "F", 15: "P", 16: "S", 17: "Cl"}


def predict_bonds(elements, pos):
    bond_index = []
    bond_type = []
    for i in range(len(elements)):
        for j in range(i + 1, len(elements)):
            p = sorted([elements[i], elements[j]])
            p = [periodic_table[int(x)] for x in p]
            l = np.linalg.norm(pos[i] - pos[j])
            bond_order = geom_predictor(p, l)
            if bond_order > 0:
                bond_index.append([i, j])
                bond_type.append(bond_order)
                bond_index.append([j, i])
                bond_type.append(bond_order)

    return np.transpose(bond_index).tolist(), bond_type
