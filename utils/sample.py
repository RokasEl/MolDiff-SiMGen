...
def seperate_outputs(outputs, n_graphs, batch_node, halfedge_index, batch_halfedge):
    ...
    for i_mol in range(n_graphs):
        ...
        new_pred_this = [
            outputs_pred[0][ind_node],  # node type
            outputs_pred[1][ind_node],  # node pos
            outputs_pred[2][ind_halfedge],
        ]  # halfedge type

        new_traj_this = [
            outputs_traj[0][:, ind_node],  # node type. The first dim is time
            outputs_traj[1][:, ind_node],  # node pos
            outputs_traj[2][:, ind_halfedge],
        ]  # halfedge type
