...
def get_drug_chem(mol):
    ...
    return {"qed": qed_score, "sa": sa_score, "logp": logp_score, "lipinski": lipinski}
...
def get_metric(mols, metric, parallel=False):
    ...
    if not parallel:
        ...
        with Pool(102) as pool:
            results = list(
                tqdm(pool.imap(func, mols), total=len(mols), desc=f"eval {metric}")
            )
        ...
        for bond_obj in smarts_list:
            ...
            if not parallel:
                ...
                with Pool(102) as pool:
                    ...
                    results_this_bond = list(
                        tqdm(pool.imap(func, mols), total=len(mols))
                    )
        ...
        for mol in tqdm(mols):
            for mol_bond in mol.GetBonds():
                ...
                if a0.GetAtomicNum() > a1.GetAtomicNum():
                    a0, a1 = a1, a0
                bond_sym = "".join(
                    [a0.GetSymbol(), str(int(mol_bond.GetBondType())), a1.GetSymbol()]
                )
        ...
        for mol in tqdm(mols):
            for atom in mol.GetAtoms():
                ...
                for r in bonds_sym_pairs_this_atom:
                    bonds_sym_pairs.append(
                        r[0] + "-" + (r[1][-1] + r[1][1:-1] + r[1][0])
                    )
...
class RingAnalyzer(object):
    ...
    def get_count_ring(self, mols):
        ...
        counts_dict = {
            "cnt_ring_type_{}".format(i): counts[:, i]
            for i in range(len(self.freq_val_rings))
        }
        return counts_dict
