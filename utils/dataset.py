...
def get_dataset(config, *args, **kwargs):
    ...
    if "split" in config:
        ...
        split = {
            k: [
                dataset.molid2idx[mol_id]
                for mol_id in mol_id_list
                if mol_id in dataset.molid2idx
            ]
...
class Drug3DDataset(Dataset):
    def __init__(self, root, path_dict, transform=None):
        ...
        self.molid2idx_path = (
            self.processed_path[: self.processed_path.find(".lmdb")] + "_molid2idx.pt"
        )
        # self.filter = filter
        ...
        if (not os.path.exists(self.processed_path)) or (
            not os.path.exists(self.molid2idx_path)
        ):
            self._process()
    ...
    def _process(self):
        ...
        # filter
        df_use = df_summary[
            df_summary["pass_size"]
            & df_summary["pass_element"]
            & (~df_summary["broken"])
            & (~df_summary["error_mol"])
        ]

        num_skipped = 0
        with db.begin(write=True, buffers=True) as txn:
            for _, line in tqdm(
                df_use.iterrows(), total=len(df_use), desc="Preprocessing data"
            ):
                # mol info
                mol_id = line["mol_id"]
                smiles = line["smiles"]
                ...
                try:
                    ...
                    suppl = Chem.SDMolSupplier(
                        os.path.join(self.sdf_path, "mol_%d.sdf" % mol_id)
                    )
                    confs_list = []
                    for i_conf in range(len(suppl)):
                        mol = Chem.MolFromMolBlock(
                            suppl.GetItemText(i_conf).replace(
                                "RDKit          3D", "RDKit          2D"
                            )
                    ...
                    txn.put(key=str(mol_id).encode(), value=pickle.dumps(data))
                except:
                    num_skipped += 1
        ...
        db.close()
        print(
            "Processed %d molecules" % (len(df_use) - num_skipped),
            "Skipped %d molecules" % num_skipped,
        )
