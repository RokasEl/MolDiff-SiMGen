...
class Config:
    ...
    default_sigma: float
    element_sigmas: dict[int, float] | None
    num_mols: int
    batch_size: int
    experiment_name: str
    max_size: int= 20
...
def main(config_path: str):
    ...
    element_sigma_array = np.ones_like(z_table.zs, dtype=np.float32) * config.default_sigma
    ...
    while len(pool['finished']) < config.num_mols:
        if len(pool['failed']) > 256:
            logging.info(f'Experiment NO. {config.experiment_name}: Too many failed molecules. Stop sampling.')
            break
